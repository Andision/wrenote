...
export function UploadDialog({ open, onClose }: UploadDialogProps) {
  // The body only exists while the dialog is open, so every open starts from
  // fresh state — no effect resetting fields behind the animation. (Reopening
  // inside the ~180 ms exit revives the same body; nothing can click that
  // fast through the fading backdrop.)
  return (
    <AnimatePresence>
      {open && <UploadDialogBody key="upload-body" onClose={onClose} />}
    </AnimatePresence>
  );
}
